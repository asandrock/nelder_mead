...
!     Changed API to return also the covariance matrix (if minimized function
!     is -LOG(LIKELIHOOD)).
!     Alexander Sandrock, 24 March 2021
...
REAL (dp), INTENT(OUT)     :: var(:), cov(:,:), func
...
call conv_tri_matrix(vc, nop, cov)
...
END SUBROUTINE print_tri_matrix
  ...
  subroutine conv_tri_matrix(a, n, b)
    real(dp), intent(in) :: a(:)
    integer, intent(in) :: n
  ...
  end subroutine conv_tri_matrix
